  ...
  return (
    <div className="relative z-10 max-w-6xl mx-auto px-6 py-8">
      <div className="flex items-end justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold">The Galaxy</h1>
          ...
          return unlocked ? (
            <div key={c.id} className="flex flex-col items-center gap-2">
              <PlayerCard
                ...
                stats={c.stats}
              />
              <span className="text-[0.65rem] text-emerald-400/90 uppercase tracking-widest">Collected</span>
            </div>
          ) : (
            <button
              key={c.id}
            ...
            >
              <div className="w-40 h-56 rounded-2xl border border-white/10 bg-white/[0.03] flex flex-col items-center justify-center gap-2 transition group-hover:border-violet-400/40 group-hover:bg-violet-500/5">
                <div className="text-5xl grayscale opacity-25 blur-[1.5px] select-none">{c.avatar}</div>
                <div className="text-slate-500 text-xs font-medium">{c.name}</div>
                <div className="text-slate-600 text-[0.65rem]">{c.role}</div>
              ...
              <span className="text-[0.65rem] text-slate-600 uppercase tracking-widest">Locked</span>
            </button>
          );
        })}
      </div>
