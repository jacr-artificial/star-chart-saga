...
/**
 * Painted door: accepts a CV file but never reads it.
 * A scripted "analysing" sequence assigns the spawn planet.
 */
export default function Onboarding() {
  const { setOnboarded, go } = useStore();
  const [stage, setStage] = useState("drop"); // drop | scanning | reveal
  ...
  const accept = (name) => {
    setFileName(name);
    setStage("scanning");
    window.setTimeout(() => setStage("reveal"), 3000); // scripted
  };
  ...
  return (
    <div className="relative z-10 min-h-screen flex flex-col items-center justify-center px-6 text-center">
      ...
      <h1 className="text-4xl font-extrabold tracking-tight">
        Welcome to <span className="bg-gradient-to-r from-violet-400 to-amber-300 bg-clip-text text-transparent">Orbit</span>
      </h1>
      <p className="text-slate-400 mt-3 max-w-md">
        Day one, {DEMO_USER.name.split(" ")[0]}. Drop your CV and we'll find your place in the galaxy.
      </p>
        ...
        <>
          <div
            ...
            className={`mt-10 w-full max-w-md rounded-3xl border-2 border-dashed px-8 py-14 cursor-pointer transition ${
              dragOver ? "border-violet-400 bg-violet-500/10 scale-[1.02]" : "border-white/20 bg-white/[0.03] hover:border-violet-400/60"
            }`}
          >
          ...
          <button
            onClick={() => { setOnboarded(true); go("galaxy"); }}
