...
import { PLANETS, QUESTIONS } from "../data.js";
...
export default function PlanetView() {
  ...
  return (
    <div className="relative z-10 max-w-4xl mx-auto px-6 py-8">
      <button onClick={() => go("galaxy")} className="text-sm text-slate-400 hover:text-slate-200">← Back to galaxy</button>
  ...
  return (
    <div className="mt-10">
      <div className="flex items-center justify-between text-sm">
        <span className="text-slate-400">Mission {idx + 1} of {QUESTIONS.length}</span>
        <span className="text-slate-400">
          <span className="text-emerald-400 font-semibold">{correctCount}</span> correct · {answeredCount}/{QUESTIONS.length} attempted
        </span>
      </div>
      <div className="mt-2 h-1.5 rounded-full bg-white/10 overflow-hidden">
        <div className="progress-fill h-full bg-gradient-to-r from-violet-500 to-amber-400" style={{ width: `${(answeredCount / QUESTIONS.length) * 100}%` }} />
      </div>
      ...
      <div className="mt-8 rounded-3xl border border-white/10 bg-white/[0.04] p-7">
        ...
        <div className="mt-5 grid gap-2.5">
          {q.options.map((opt, i) => {
            let cls = "border-white/10 bg-white/5 hover:border-violet-400/50 hover:bg-violet-500/10";
            if (state) {
        ...
        {state && (
          <div className={`mt-4 text-sm font-medium ${state.correct ? "text-emerald-400" : "text-rose-400"}`}>
            {state.correct ? `✨ Correct — +${q.xp} XP for you and Squad Nebula` : "Not quite — the right answer is highlighted"}
          </div>
        )}
      </div>
