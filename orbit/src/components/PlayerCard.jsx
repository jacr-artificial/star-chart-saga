...
import { RARITY_STYLES, STAT_KEYS } from "../data.js";

/**
 * The trading-card visual, reused across editor / collection / collect flow.
 * Flips on click when `flippable`.
 */
export default function PlayerCard({
  name, role, avatar, rarity, catchphrase, stats,
  size = "md", flippable = true, holo = true, className = "",
}) {
      ...
      <div className={`card-inner relative w-full h-full ${flipped ? "flipped" : ""}`}>
        ...
        >
          <div className="w-full h-full rounded-xl bg-[#0b0d22]/85 flex flex-col items-center px-3 pt-4 pb-3">
            <div className={`self-end px-2 py-0.5 rounded-full border ${r.chip} ${textScale} font-semibold tracking-wide`}>
              {r.label}
            </div>
            <div className={`${size === "sm" ? "text-4xl mt-1" : "text-6xl mt-3"} drop-shadow-[0_0_12px_rgba(167,139,250,0.5)]`}>
              {avatar}
            </div>
            <div className={`mt-2 font-bold ${size === "sm" ? "text-sm" : "text-lg"} text-center leading-tight`}>{name}</div>
            <div className={`${textScale} text-slate-400 text-center`}>{role}</div>
            <div className={`mt-auto w-full grid grid-cols-2 gap-1.5`}>
              {STAT_KEYS.map((s) => (
                <div key={s.key} className={`flex items-center gap-1 bg-white/5 rounded-md px-1.5 py-1 ${textScale}`}>
                  <span>{s.icon}</span>
        ...
        >
          <div className="w-full h-full rounded-xl bg-[#0b0d22]/90 flex flex-col items-center justify-center px-4 text-center">
            ...
            <div className={`italic ${size === "sm" ? "text-xs" : "text-sm"} text-slate-200 leading-relaxed`}>
              “{catchphrase || "…"}”
            </div>
            <div className={`mt-4 ${textScale} text-slate-500 uppercase tracking-widest`}>catchphrase</div>
