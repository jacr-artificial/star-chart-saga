...
export default function PlanetView() {
  ...
  return (
    <div className="relative z-10 max-w-4xl mx-auto px-6 py-8">
      <button onClick={() => go("galaxy")} className="text-sm text-muted-foreground hover:text-foreground">
        ← Back to galaxy
      </button>
      <div className="mt-4 flex items-center gap-6">
        ...
        <div>
          ...
          <div className="text-sm text-brand-magenta">{planet.domain}</div>
          <p className="text-sm text-muted-foreground mt-1">{planet.blurb}</p>
        </div>
      </div>
      ...
      {planet.real ? (
        ...
        <div className="mt-12 rounded-3xl border border-brand-panel bg-white/[0.03] px-8 py-16 text-center">
  ...
  return (
    <div className="mt-10">
      <div className="flex items-center justify-between text-sm">
        ...
        <span className="text-muted-foreground">
          <span className="text-brand-lemon font-semibold">{correctCount}</span> correct ·{" "}
          {answeredCount}/{QUESTIONS.length} attempted
        </span>
      </div>
      <div className="mt-2 h-1.5 rounded-full bg-white/10 overflow-hidden">
        <div
          className="progress-fill h-full bg-gradient-to-r from-brand-magenta to-brand-lemon"
          style={{ width: `${(answeredCount / QUESTIONS.length) * 100}%` }}
        />
      </div>

      <div className="mt-8 rounded-3xl border border-brand-panel bg-white/[0.04] p-7">
        <div className="text-xs text-brand-lemon/90 uppercase tracking-widest">+{q.xp} XP</div>
        ...
        <div className="mt-5 grid gap-2.5">
          {q.options.map((opt, i) => {
            ...
            if (state) {
              if (i === q.answerIdx) cls = "border-brand-lemon/70 bg-brand-lemon/15";
              else if (i === state.pickedIdx) cls = "border-brand-magenta/70 bg-brand-magenta/15";
              else cls = "border-brand-panel/60 bg-white/[0.02] opacity-60";
            }
            return (
              <button
                key={i}
              ...
              >
                <span className="text-muted-foreground/80 mr-2">{String.fromCharCode(65 + i)}.</span>
                {opt}
              </button>
            );
          })}
        ...
        {state && (
          <div
            className={`mt-4 text-sm font-medium ${state.correct ? "text-brand-lemon" : "text-brand-magenta"}`}
          >
            {state.correct
      ...
      <div className="mt-5 flex justify-between">
        <button
          ...
          disabled={idx === 0}
          className="px-4 py-2 rounded-xl text-sm border border-brand-panel text-foreground/90 disabled:opacity-30 hover:bg-white/5"
        >
          ← Previous
        </button>
        <button
